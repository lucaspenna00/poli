...
int main() {
    ...
    while (1) {
        ...
        if ( tx_nrf24l01p.readable() ) {

              rxDataCnt = tx_nrf24l01p.read( NRF24L01P_PIPE_P0, rxData, sizeof( rxData ));

              // Display the receive buffer contents via the host serial lin

              int num_pulsos;

              sscanf(rxData, "%d", &num_pulsos);

              printf("\n\r");

              float distancia;

              printf("[INFO] Numero de Pulsos: %d \n\r", num_pulsos);

              distancia =  num_pulsos/76.0;

              printf("[INFO] Distancia percorrida: %f [m]\n\r", distancia);
        }
